...
class Firewall(EventMixin):
    ...
    def __init__(self, l2config, l3config):
        ...
        with open(l3config) as csvfile:
            ...
            for row in self.rules:
                ...
                s_port = row['src_port']
                d_port = row['dst_port']
                print 'src_ip, dst_ip, src_port, dst_port', s_ip, d_ip, \
                    s_port, d_port
    ...
    def add_rule_to_l3_config(self, srcmac='any', srcip='any', dstip='any'):
        with open(l3config, 'a') as file:
            log.debug("Writing l3config file !")
            ...
            log.debug("Written: srcip=%s dstip=%s srcmac=%s" % (str(srcip), str(dstip), str(srcmac)))

    def prevent_ip_spoofing(self, packet, match=None, event=None):
        '''
        Prevents IP Spoofing by comparing incoming IP and the IP retrieved from SpoofingHashMap
        using the incoming MAC address
        '''
        ip_packet = packet.payload
        if packet.type == packet.IP_TYPE:
            if packet.src in self.SpoofingHashMap:
                prev_ip = self.SpoofingHashMap.get(packet.src)[0]
                prev_port = self.SpoofingHashMap.get(packet.src)[1]
                if prev_ip != ip_packet.srcip:
                    log.debug(
                        "IP spoofing attempt!! Packet with MAC %s previously seen! [IP %s on port %s]" %
                        (str(packet.src), str(prev_ip), str(prev_port)))
                    srcmac = str(packet.src)
                    dstip = str(ip_packet.dstip)
                    self.add_rule_to_l3_config(srcmac, 'any', dstip)
                    return True
            else:
                self.SpoofingHashMap[packet.src] = [ip_packet.srcip, ip_packet.dstip, event.port]
        return False
    ...
    def _handle_PacketIn(self, event):
        ...
        if match.dl_type == packet.ARP_TYPE and match.nw_proto \
            == arp.REQUEST:
        ...
        if match.dl_type == packet.IP_TYPE:
            ...
            if self.prevent_ip_spoofing(packet, match, event):
                log.debug("IP spoofing detected - block flow...")
            else:
                log.debug("No spoofing detected - allow flow...")
